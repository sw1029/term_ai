...
기본적으로 실험 실행은 resume을 켭니다. 중단 또는 세션 종료 후 같은
`execution.output_dir`로 다시 실행하면 가능한 경우 기존 진행분과 checkpoint를
이어 씁니다.

G0~G4 생성형 실험은 기본적으로 tokenizer chat template를 사용합니다. 새 output
dir에서 실행한 prediction/metric 로그에는 `prompt_mode_effective`,
`prompt_contract_version`, `parser_version`, `parse_method`,
`strict_parse_error`가 기록됩니다. 기존 `runs/` 산출물은 자동 재채점하지 않으며,
legacy plain prompt 비교가 필요할 때만 `execution.prompt_mode=plain` 또는
standalone CLI의 `--prompt-mode plain`을 지정합니다.

주요 진행/복구 산출물:

- `resume_state.json`: 현재 stage, 완료 개수, 최신 checkpoint, 최종 artifact
- `prediction_log.partial.jsonl`: item 단위 partial prediction
- `metric_log.partial.json`: partial metric snapshot
- `metric_history.jsonl`: epoch/step/log/eval/trial별 metric history
- `checkpoints/`: Trainer 또는 수동 PyTorch epoch checkpoint
- `backups/`: timestamped checkpoint/final weight backup

새로 시작해야 할 때는 `execution.resume=false`를 지정합니다. standalone CLI는
`--no-resume`을 사용합니다. Trainer 계열은 `training.resume=false`로 checkpoint
load만 끌 수 있고, `training.save_steps`, `training.save_total_limit`,
`training.backup_checkpoints`로 checkpoint 주기를 조정할 수 있습니다.

`Ctrl-C`와 `SIGTERM`은 interrupt guard가 마지막 partial metric/state와 가능한
checkpoint 정보를 저장합니다. `SIGKILL`, 전원 종료, OS crash처럼 Python handler가
실행되지 않는 종료는 마지막 flush/checkpoint까지만 복구됩니다.
